  ...
  return (
    <div className="min-h-screen pt-32 px-8 bg-[#fdf8f3] pb-24">
      <div className="max-w-2xl mx-auto">
        <Link href="/marketplace" className="text-[10px] uppercase tracking-[0.2em] font-black accent-gradient-text mb-8 inline-block">
          ← Back to Marketplace
        </Link>
            ...
            <Card className="rounded-[32px] border-0 shadow-xl shadow-[#262626]/5">
              <CardContent className="p-10">
                <form onSubmit={handleKYC} className="space-y-6">
                  ...
                  <Button type="submit" disabled={loading} className="w-full bg-[#262626] text-white rounded-full py-8 text-[12px] font-black uppercase tracking-[0.2em] hover:accent-gradient hover:text-[#262626] transition-all">
            ...
            <div className="grid grid-cols-3 gap-4 mb-12">
              {[
                ...
                <button 
                  ...
                  className={`p-6 rounded-[24px] border-2 transition-all flex flex-col items-center gap-4 ${type === item.id ? 'accent-gradient-border accent-gradient/10' : 'border-[#262626]/5 bg-white'}`}
                >
                  <item.icon className={`w-8 h-8 ${type === item.id ? 'accent-gradient-text' : 'text-[#262626]/20'}`} />
            ...
            <Card className="rounded-[32px] border-0 shadow-xl shadow-[#262626]/5">
              <CardContent className="p-10">
                <form onSubmit={handleCreate} className="space-y-6">
                  ...
                  {type === 'vehicle' && (
                    <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase tracking-[0.2em] accent-gradient-text">Security Deposit (Refundable) (₹)</label>
                      <Input name="security_deposit_amount" type="number" placeholder="2000" className="rounded-full px-6 py-6 accent-gradient-border/20" />
                    </div>
                  )}
                  <div className="space-y-2">
                  ...
                  <Button type="submit" disabled={loading || !type} className="w-full accent-gradient text-[#262626] rounded-full py-8 text-[12px] font-black uppercase tracking-[0.2em] hover:opacity-80 transition-all flex items-center justify-center gap-3">
